...
if [ "$TRAVIS_TAG" ]; then
  ...
  npm run dist
  cd temp_web
  git clone https://$ROT_TOKEN@github.com/joshualzb-jstk/lib.git && cd lib
  ...
  # build theme-chalk
  cd temp_web
  git clone https://$ROT_TOKEN@github.com/joshualzb-jstk/theme-chalk.git && cd theme-chalk
  rm -rf *
  ...
  cd temp_web
  git clone --depth 1 -b gh-pages --single-branch https://$ROT_TOKEN@github.com/joshualzb-jstk/lz-component-b.git && cd element
  # build sub folder
  echo $TRAVIS_TAG
...
cd temp_web
git clone https://$ROT_TOKEN@github.com/joshualzb-jstk/dev.git && cd dev
mkdir $TRAVIS_BRANCH
rm -rf $TRAVIS_BRANCH/**
...
cd temp_web
git clone -b $TRAVIS_BRANCH https://$ROT_TOKEN@github.com/joshualzb-jstk/theme-chalk.git && cd theme-chalk
rm -rf *
cp -rf ../../packages/theme-chalk/** .
git add -A .
git commit -m "$TRAVIS_COMMIT_MSG"
